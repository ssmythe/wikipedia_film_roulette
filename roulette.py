...
def simplify_label(label, country):
    label = label.strip()
    # Remove leading country name if it exists (as a prefix)
    if label.lower().startswith(country.lower()):
        label = label[len(country) :].strip()
    # Remove trailing 'films' word
...
def get_final_film_titles(url, desired_subgenre=None):
    if desired_subgenre:
        ...
        if match:
            ...
            verbose_print(
                f"Using specified subgenre: {subgenre_title.replace('_', ' ')}"
            )
            films = get_film_titles_from_live_page(subgenre_url, category="subgenre")
            return films, simplify_label(
                subgenre_title.replace("_", " "), country_slug.replace("_", " ")
            )
        print(
            f"Error: Could not determine subgenre URL for '{desired_subgenre}'.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    ...
    while len(results) < args.n and attempts < max_attempts:
        ...
        if args.g:
            ...
            genre_links = get_genre_links_from_live_page(
                chosen_country["genre_page_url"]
            )
